...
export default function Categories() {
  const { data, error } = useSWR('/api/categories', (...args) => fetch(...args).then(res => res.json()))
  if (error) return <div>failed to load</div>
  if (!data) return <div>loading...</div>
  // const action = (e) => {
  //   useSWR('/api/categories/' + parseInt(e.currentTarget.dataset.id) + '/' + e.currentTarget.dataset.oper)
  //     .then(res => {
  //       console.log(res)
  //     })
  // }
                    ...
                    return (
                      <tr>
                        <th className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-left">
                          {obj.nome}<br /><small>{obj.tipo}</small>
                        </th>
                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                          <ActionButton icon="trash" color="lightGrey" action={teste} data-id={obj.id} data-oper="delete" />
                        </td>
                      </tr>
                    )
                  })}
